...
function RecipeChat() {
  ...
  useEffect(() => {
    ...
    if (generatedRecipes) setIsFromGenerated(true);
    ...
    if (storedRecipe) {
      try {
        ...
        const initialMessage: ChatMessage = {
          role: 'assistant',
          content: `I'm your recipe assistant for "${parsedRecipe.name}". Ask me to modify ingredients, adjust portions, make it spicier, suggest substitutions, or answer questions!`,
          timestamp: new Date(),
        };
      ...
      console.error('No recipe found in session storage');
      router.push('/recipes');
    }
  }, [router]);
...
Prep/Cook Times: ${recipe.times}
      ...
      toast.error('Failed to get response', { description: 'Try again or refresh' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleReset = () => {
    if (recipe) {
      const initialMessage: ChatMessage = {
        role: 'assistant',
        content: `I'm your recipe assistant for "${recipe.name}". Ask me to modify ingredients, adjust portions, make it spicier, suggest substitutions, or answer questions!`,
        timestamp: new Date(),
      };
      setMessages([initialMessage]);
      toast.success('Conversation reset', { description: 'Starting anew' });
    }
  };

  const handleBackToRecipes = () => {
    router.push(isFromGenerated ? '/recipes/results' : '/recipes');
  };

  const handleSaveModifiedRecipe = () => {
    toast.success('Coming Soon', { description: 'Saving modified recipes will be available soon!' });
  };
            ...
            <Card>
              <CardHeader className='pb-2'>
                <div className='flex items-center justify-between'>
                  ...
                  <Button variant='ghost' size='sm' className='lg:hidden' onClick={toggleRecipeExpansion}>
                    <Minimize className='h-4 w-4' />
                  </Button>
                </div>
                <CardDescription>Reference the original recipe</CardDescription>
              </CardHeader>
              <CardContent className='p-0'>
                <ScrollArea className='h-[calc(100vh-350px)] min-h-[400px]'>
                  <div className='p-4 space-y-4'>
                    <div>
                      <h2 className='text-xl font-bold text-gray-900 dark:text-gray-100 mb-2'>{recipe.name}</h2>
                    ...
                    <div>
                      ...
                      <p className='text-sm text-gray-600 dark:text-gray-400'>{recipe.nutritionalFacts}</p>
                    </div>
                  </div>
                </ScrollArea>
              </CardContent>
          ...
          <div className={`${expandedRecipe ? 'col-span-1 lg:col-span-2' : 'col-span-1 lg:col-span-2'}`}>
            ...
            <Card className='mb-4'>
              <CardHeader className='pb-3'>
                ...
                <CardDescription>Ask questions or modify "{recipe.name}"</CardDescription>
              </CardHeader>
              <CardContent className='p-0'>
            ...
            <Alert className='mt-4 bg-blue-50 dark:bg-blue-900/20 border-blue-200 dark:border-blue-800'>
              ...
              <AlertDescription className='text-blue-800 dark:text-blue-300'>
                Try asking: "Can I make this vegetarian?", "Adjust for 8 people?", or "Substitute for [ingredient]?"
              </AlertDescription>
            </Alert>
          </div>
        </div>
